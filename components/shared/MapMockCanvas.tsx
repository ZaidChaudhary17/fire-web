...
import { GoogleMapsGpsCanvas } from "./GoogleMapsGpsCanvas";
...
interface MapMockCanvasProps {
  ...
  heightClass?: string;
  defaultMode?: "google_maps" | "vector_gis";
}
...
export function MapMockCanvas({
  ...
  onSelectVehicle,
  heightClass = "h-[500px]",
  defaultMode = "google_maps",
}: MapMockCanvasProps) {
  const [viewMode, setViewMode] = React.useState<"google_maps" | "vector_gis">(defaultMode);
  ...
  return (
    <div className="flex flex-col space-y-2">
      ...
      <div className="flex items-center justify-between px-1">
        <div className="flex items-center gap-2">
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1 rounded-lg text-xs font-bold transition shadow-sm ${
              viewMode === "google_maps"
                ? "bg-[#0a2540] text-white border border-[#0a2540]"
                : "bg-white text-slate-700 border border-slate-300 hover:bg-slate-50"
            }`}
          >
            <Globe className="w-3.5 h-3.5 text-cyan-400" />
            <span>Google Maps GPS (Live)</span>
          </button>
          ...
          >
            ...
            <span>Municipal Survey Grid</span>
          </button>
        </div>

        <span className="text-[11px] font-semibold text-slate-500 hidden sm:inline">
          Centroid: 19.2812° N, 72.8561° E
        </span>
      </div>
      ...
      {viewMode === "google_maps" ? (
        ...
        /* Render Municipal Vector GIS View */
